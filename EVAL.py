...
from SCORE_BOARD import Ui_Dialog as Score  
import sqlite3
match= sqlite3.connect("cricdb.db")
...
"font: 75 italic 11pt \"Century Gothic\";")
    ...
    def retranslateUi(self, evaluate_team):
        ...
        m = matchcur.execute("SELECT  DISTINCT name from teams;")
        team = m.fetchall()
        for q in team:
            self.selectteam_cb.addItem(q[0])
    def changedname(self, w):
        self.players_lw.clear()
        self.scores_lw.clear()
        b = matchcur.execute("SELECT players from teams WHERE name='" + w + "';")
        player = b.fetchall()
        
        
        for j in player:
            self.players_lw.addItem(j[0])
        n = matchcur.execute("SELECT value from teams WHERE name='" + w + "';")
        value = n.fetchall()
        for f in value:
            self.scores_lw.addItem(str(f[0]))


    def final_score(self):
        total_score=0
        w=self.selectteam_cb.currentText()   # TEAMNAME
        n = matchcur.execute("SELECT value from teams WHERE name='" + w + "';")
        value = n.fetchall()
        for f in value:
            total_score+=f[0]
        self.score_screen.finalscore.setText(str(total_score))  # OPENING SCORE DIALOG BOX AND SET FINAL SCORE
