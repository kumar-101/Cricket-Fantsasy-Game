...
# Form implementation generated from reading ui file 'FINALCODE.ui'
#
...
from CALCULATE import player_points
from PyQt5 import QtCore, QtGui, QtWidgets
from PyQt5.QtWidgets import QMessageBox



from OPENTEAM import Ui_Dialog as Open   
from NEWTEAM import Ui_Dialog as New     
from EVAL import Ui_evaluate_team as Eval

import sqlite3
f=sqlite3.connect('cricdb.db')  
fcurs=f.cursor()
...
# INITIALIZING LISTS
        ...
        self.c = []        # ALLROUNDER 
        self.d = []        # WICKETKEEPER
        self.list1 = []    # SELECTED PLAYERS

        
        MainWindow.setObjectName("MainWindow")
        MainWindow.resize(544, 523)
...
"border-color: rgb(255, 255, 255);")
        ...
        self.bat_rb = QtWidgets.QRadioButton(self.frame_5)      # BATSMAN
        ...
        self.bow_rb = QtWidgets.QRadioButton(self.frame_5)      #BOWLER
        sizePolicy = QtWidgets.QSizePolicy(QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Fixed)
        sizePolicy.setHorizontalStretch(0)
        ...
        self.horizontalLayout_2.addWidget(self.bow_rb)          # ALL ROUNDER
        ...
        self.wk_rb = QtWidgets.QRadioButton(self.frame_5)       # WICKET KEEPER
        sizePolicy = QtWidgets.QSizePolicy(QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Fixed)
        sizePolicy.setHorizontalStretch(0)
        sizePolicy.setVerticalStretch(0)
        self.ar_rb.setEnabled(False)
        ...
        self.verticalLayout.addWidget(self.frame_5)             # AVAILABLE PLAYERS
        ...
        self.verticalLayout_2.addWidget(self.frame_6)  #SELECT PLAYERS
        ...
        self.menubar = QtWidgets.QMenuBar(MainWindow)  #MENU BAR
...
"")
        ...
        self.actionQuit = QtWidgets.QAction(MainWindow)# EXIT 
...
# NEW FILE 
    ...
    def namechange(self):
        ...
        fcurs.execute("SELECT DISTINCT name FROM teams")
        b = fcurs.fetchall()
        for k in b:
            if k[0] == teamname:
    ...
    def file_save(self):
        if not self.error():  
            ...
            try:
                ...
                z = fcurs.fetchall()
                for l in z:
                    if self.team_name.text() == l[0]:   # CHECKING IF THE TEAMNAME ALREADY EXISTS
                        print('Updating already there')
                        fcurs.execute("DELETE  FROM teams WHERE name='" + self.team_name.text() + "';") #DELETING TEAM
            except:
                print('error')
            for l in range(self.sel_player.count()):
                
                try:
                    fcurs.execute("INSERT INTO teams (name,players,value) VALUES (?,?,?)",
                                     (self.team_name.text(), self.list1[l], player_points[self.list1[l]]))
                except:
...
# RADIOBUTTONS CLICK
    ...
    def conditions_1(self, cat):     # CALCULATING POINTS
    ...
    def conditions_2(self, cat):     # CALCULATING POINTS
        self.avail_points += self.stats[cat]
        self.used_points -= self.stats[cat]
        if cat in self.a:
            self.bowlerscount -= 1
    ...
    def openteam(self):              # OPEN TEAM
        ...
        self.enablebuttons()
        fcurs.execute("SELECT players from teams WHERE name= '" + teamname + "';")
        x=fcurs.fetchall()
        score=[]
        for i in x:
            fcurs.execute("SELECT value from stats WHERE player='"+i[0]+"';")
            y=fcurs.fetchone()
            score.append(y[0])
        sum=0
        for i in score:
